...
if (!function_exists('pcf_render_item_card')) {
    function pcf_render_item_card(array $item, int $width = 180, bool $preferFullPackageImage = false): void
    {
        ...
        $sampleImageClass = $hasSampleImages && $contentId !== ''
            ? 'pcf-dm-card__button sample-button sample-button--enabled sample-image-trigger'
            : 'pcf-dm-card__button sample-button sample-button--disabled sample-image-trigger';
        ...
        echo '<button type="button" class="' . e($sampleImageClass) . '"' . (!$hasSampleImages || $contentId === '' ? ' disabled' : '') . ' data-sample-images-url="' . e($sampleImagesUrl) . '" data-sample-images-title="' . e($title) . '">サンプル画像</button>';
        echo '</div>';
        echo '</article>';
    }
}
